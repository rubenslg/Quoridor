...
// BFS: verifică dacă jucătorul are cel puțin un drum liber spre ținta sa
function areCaleDrumCatreTinta(jucator) {
    let vizitat = Array.from({ length: randuri }, () => new Array(coloane).fill(false));
    let coada = [{ x: jucator.x, y: jucator.y }];
    vizitat[jucator.y][jucator.x] = true;
    let directii = [{ dx: 0, dy: -1 }, { dx: 0, dy: 1 }, { dx: -1, dy: 0 }, { dx: 1, dy: 0 }];

    while (coada.length > 0) {
        let { x, y } = coada.shift();
        if (y === jucator.targetY) return true;
        for (let { dx, dy } of directii) {
            let nx = x + dx, ny = y + dy;
            if (nx >= 0 && nx < coloane && ny >= 0 && ny < randuri && !vizitat[ny][nx] && faraZidInCale(x, y, nx, ny)) {
                vizitat[ny][nx] = true;
                coada.push({ x: nx, y: ny });
            }
        }
    }
    return false;
